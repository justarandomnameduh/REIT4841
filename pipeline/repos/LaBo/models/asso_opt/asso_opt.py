...
class AssoConcept(pl.LightningModule):
    ...
    def __init__(self, cfg, init_weight=None, select_idx=None) -> None:
        ...
        if not concept_feat_path.exists():
            ...
            self.concept_raw = np.load(concept_raw_path, allow_pickle=True)[self.select_idx]
            concept2cls_arr = np.load(concept2cls_path, allow_pickle=True)[self.select_idx]
            # Ensure it's always an array even if indexing returns scalar
            if concept2cls_arr.ndim == 0:
                concept2cls_arr = np.array([concept2cls_arr])
            self.concept2cls = th.from_numpy(concept2cls_arr).long().view(1, -1)
        ...
        self.train_acc = torchmetrics.Accuracy(task='multiclass', num_classes=cfg.num_cls)
        self.valid_acc = torchmetrics.Accuracy(task='multiclass', num_classes=cfg.num_cls)
        # self.test_acc = torchmetrics.Accuracy(task='multiclass', num_classes=cfg.num_cls, average='macro')
        self.test_acc = torchmetrics.Accuracy(task='multiclass', num_classes=cfg.num_cls)
        
        # F1 score for imbalanced classes
        self.train_f1 = torchmetrics.F1Score(task='multiclass', num_classes=cfg.num_cls, average='macro')
        self.valid_f1 = torchmetrics.F1Score(task='multiclass', num_classes=cfg.num_cls, average='macro')
        self.test_f1 = torchmetrics.F1Score(task='multiclass', num_classes=cfg.num_cls, average='macro')
        
        self.all_y = []
        self.all_pred = []
        self.confmat = torchmetrics.ConfusionMatrix(task='multiclass', num_classes=self.cfg.num_cls)
