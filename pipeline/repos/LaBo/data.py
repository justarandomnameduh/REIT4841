...
class DotProductDataset(Dataset):
    ...
    def __init__(self, img_feat, txt_feat, label, on_gpu):
        ...
        # Keep data on CPU for multi-process data loading compatibility
        # PyTorch Lightning will automatically move it to GPU
        self.dot_product = self.dot_product.cpu()
        self.labels = label.cpu()
...
class DataModule(pl.LightningDataModule):
    ...
    def __init__(
        ...
        self.cls_names = np.load(cls_names_path, allow_pickle=True).tolist() # for reference, the mapping between indices and names
        ...
        self.concepts_raw = np.load(concept_raw_path, allow_pickle=True)
        self.concept2cls = np.load(concept2cls_path, allow_pickle=True)
