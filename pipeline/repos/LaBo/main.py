...
def asso_opt_main(cfg):
    ...
    if not cfg.DEBUG:
        ...
        # Create run name with k, n, c if available (for HAM10000), otherwise use default
        if hasattr(cfg, 'k_clusters') and hasattr(cfg, 'n_images') and hasattr(cfg, 'c_concepts') and hasattr(cfg, 'vlm_model'):
            run_name = f'{proj_name}_k{cfg.k_clusters}_n{cfg.n_images}_c{cfg.c_concepts}_{cfg.vlm_model}'
        else:
            run_name = '{}shot_{}_{}_{}'\
                       .format(cfg.n_shots, cfg.concept_type, cfg.num_concept, cfg.submodular_weights[1])
        
        wandb_logger = pl.loggers.WandbLogger(name=run_name,
                       project=proj_name.upper() if proj_name.lower() in ['ham10000', 'derm7pt', 'cub'] else proj_name,
        ...
        trainer = pl.Trainer(accelerator='gpu',
                             ...
                             enable_model_summary=False,  # Disable verbose summary
