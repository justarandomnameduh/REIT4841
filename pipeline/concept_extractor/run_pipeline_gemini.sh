...
K_VALUES=(1 3 5 10)
N_VALUES=(1 2 4)
C_VALUES=(1 3 5 10 30 50 100)

echo "========================================"
echo "Concept Extraction Pipeline - GEMINI"
echo "========================================"
echo ""
    ...
    for k in "${K_VALUES[@]}"; do
        for n in "${N_VALUES[@]}"; do
            for c in "${C_VALUES[@]}"; do
                if [ -f "concepts/gemini/$dataset/concept_all_${k}_${n}_${c}.json" ]; then
                    existing=$((existing + 1))
                else
                    missing=$((missing + 1))
                fi
...
for DATASET in "${DATASETS[@]}"; do
    ...
    for K in "${K_VALUES[@]}"; do
        ...
        for N in "${N_VALUES[@]}"; do
            ...
            PARAGRAPH_FILE="clusters/gemini/$DATASET/paragraph_${K}_${N}.json"
            if [ -f "$PARAGRAPH_FILE" ]; then
                echo ""
                echo "  [3/5] Paragraph generation already exists: $PARAGRAPH_FILE (skipping)"
            else
                echo ""
                echo "  [3/5] Generating cluster paragraphs with Gemini..."
                python gen_paragraph_gemini.py --dataset "$DATASET" --k "$K" --n "$N" --delay 2.0
            ...
            for C in "${C_VALUES[@]}"; do
                CONCEPT_FILE="concepts/gemini/$DATASET/concept_all_${K}_${N}_${C}.json"
                if [ -f "$CONCEPT_FILE" ]; then
                    echo "    ✓ Concept extraction already exists: concept_all_${K}_${N}_${C}.json (skipping)"
                else
                    echo "    Extracting $C concepts..."
                    python gen_concepts_gemini.py --dataset "$DATASET" --k "$K" --n "$N" --c "$C"
            ...
            echo "    - Paragraphs: clusters/gemini/$DATASET/paragraph_${K}_${N}.json"
            echo "    - Concepts: concepts/gemini/$DATASET/concept_all_${K}_${N}_[c].json"
        done
...
for DATASET in "${DATASETS[@]}"; do
    echo ""
    echo "$DATASET:"
    CONCEPT_DIR="concepts/gemini/$DATASET"
